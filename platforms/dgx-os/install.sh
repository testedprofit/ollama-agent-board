...
if [[ "$ARCH" != "aarch64" && "$ARCH" != "arm64" && "${ALLOW_NON_ARM64:-0}" != "1" ]]; then
  echo "DGX OS profile expects ARM64. Detected: $ARCH"
...
install_os_tools() {
  if command -v apt-get >/dev/null 2>&1; then
    ...
    echo "DGX OS should provide apt-get. Install ca-certificates curl git tar xz-utils zstd manually."
  fi
}
...
echo ""
echo "DGX OS install complete."
echo "Run: bash platforms/dgx-os/run.sh"
