...
if ! command -v npm >/dev/null 2>&1; then
  echo "npm is missing. Run: bash platforms/dgx-os/install.sh"
  exit 1
fi

if ! command -v ollama >/dev/null 2>&1; then
  echo "Ollama is missing. Run: bash platforms/dgx-os/install.sh"
  exit 1
fi
...
if [[ "$MODE" == "dev" ]]; then
  npm run dev -- --host "$HOST" --port "${DEV_PORT:-5173}"
else
  npm run build
  PORT="$PORT" npm start
fi
